...
export function usePipeCalculator(length, material, size, schedule, pipes, type) {
    // 対象のパイプデータ検索
    const pipe = computed(() => pipes.find(p => p.material === material &&
        p.size === size &&
        p.schedule === schedule &&
        (!type || p.type === type)));
    // 実重量（重量/メートル × 長さ[m]）
...
//# sourceMappingURL=usePipeCalculator.js.map